"""Learning tutor: Socratic flow, recall/assess before exercises, progress persistence."""
...
_TOOL_REQ: Dict[str, frozenset[str]] = {
    "assess_knowledge": frozenset({"learner_id", "topic"}),
    "generate_exercise": frozenset({"learner_id", "topic", "difficulty"}),
    "store_progress": frozenset({"learner_id", "topic", "outcome"}),
    "recall_history": frozenset({"learner_id"}),
}
_MUTATING = frozenset({"assess_knowledge", "store_progress"})
...
class LearningTutorAgent:
    def __init__(self, config: TutorAgentConfig, tools: Dict[str, ToolHandler]) -> None:
        self.config = config
    ...
    def _log_step(self, **kw: Any) -> None:
        log.info("%s", json.dumps({"agent": "06-learning-tutor", **kw}))

    def _validate(self, name: str, args: Dict[str, Any]) -> Optional[str]:
        need = _TOOL_REQ.get(name)
        if not need:
            return "UNKNOWN_TOOL"
        if not need.issubset(args.keys()):
            return "INVALID_INPUT"
        if args.get("learner_id") != self.config.learner_id:
    ...
    def _invoke_tool(self, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._goto(AgentState.EXECUTING)
        self._log_step(
            tool=name, elapsed_ms=int((time.perf_counter() - t0) * 1000), state=self._state.name
        )
        ...
        if name in _MUTATING and out.get("ok", True):
            self.session.mutation_log.append(dict(rec, rollback_hint="invert_progress_store"))
        return out
